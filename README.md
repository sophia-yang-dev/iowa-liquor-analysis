...
* Focusing on consumer perspectives:
  * Identify the most popular brands and types of liquor, drinking preferences in different regions and the impact of different characteristics on sales.
  * Explore trends in liquor prices over time, investigate the pricing strategies behind these trends
* From the seller's perspective:
  * Analyze how geographic location or population size influences liquor sales.
* From the social perspective:
  * Explore whether liquor sales performance is linked to local crime behavior.

### 1.2 Challenges

* First, we need to identify and collect useful, high-quality datasets. Data preprocessing and cleaning will be crucial, including handling missing or anomalous data. We must also decide how to organize and integrate different data tables effectively.
* During the analysis phase, we will face the task of formulating valuable questions and implementing analyses with clear and insightful visualizations. 
* Besides, choosing the right tools and techniques will be another key decision point, requiring careful consideration of their suitability for our objectives.

### 1.3 Datasets

To increase the complexity and workload of this analysis, we will integrate the primary dataset (Iowa liquor sales) with three additional datasets: Dataset 1 (map data or API, with source link), Dataset 2 (population data with source link), and Dataset 3 (Vin-collected crime data).
#**TODO**
...
#### 2.1.1 Overview of the Original Data
...
#### 2.1.2 Basic Data Cleaning Process
...
#### 2.1.3 Data Completion Process

We found that not all records has 'Store Location' column, but it is important for geographic location-related analysis, and we know the other column can help us to find the missing value in some data, besides, we can use the external sourses like OpenStreetMap to fill the data. So we use the following steps to complete our data:

* Step 1: Fill Location Data from Existing Dataset
  * selected a table of unique stores with name and location columns, and filtered out the stores of which the locations are empty.
  * Join back this reference table with the original dataset, and fill in the location info from the reference table whenever it is empty.(successfully filled in 487,380 records)
* Step 2: Fill Location Data from OSM with Address and City Info
  * Filtered the records with missing locations and grouped by store name.
  * Used the python library geopy to access OpenStreetMap’s geocoding service to get the location coordinates and save them as a reference table 2.
  * Join table 2 back as described in step 1 to update the location.(successfully filled in 1,381,871 records)

#### 2.1.4 Summary 

* The raw data has 30522849 records, and the final data has 30293548 records, we delete 229301 abnormal records.
* In data completion Process, We complete 1869251 store location records.

[See More Details](Data_Processing_Cleaning/DataCleaning_README.md#data_processing_cleaning) 

### 2.2 Data Analysis

#### 2.2.1 General Analysis on Sales
...
#### 2.2.2 Analysis from Time Perspective
...
#### 2.2.4 Crime and Liquor Consumption Analysis
...
### 3.1 Problems in Bacic Data Processing and Spark running

- Data Loading Issues:When using PySpark to load CSV files, automatic type inference caused mismatched data types in some fields, such as the date field being incorrectly identified as a string.To solve this, a custom schema was explicitly defined to specify the correct types for each field (e.g., StringType, DoubleType, DateType) during data loading, preventing issues caused by automatic inference.
...
### 3.2 Problems in Geographic data Processing
...
### 3.4 Problems in Machine Learning Methods Application
...
### 4.1 General Analysis Results on Sales
...
### 4.2 Time Perspective Analysis Results
...
### 4.4 Crime and Liquor Consumption Analysis Results
